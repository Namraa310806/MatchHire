...
class UserAdmin(DjangoUserAdmin):
	...
	add_fieldsets = (
		(
			...
			{
				"classes": ("wide",),
				"fields": ("email", "full_name", "role", "password1", "password2", "is_staff", "is_active", "is_verified"),
			},
		),
	)
...
class CandidateProfileAdmin(admin.ModelAdmin):
	list_display = ("user", "headline", "current_location", "years_of_experience", "resume_uploaded", "created_at")
	list_select_related = ("user",)
	search_fields = ("user__email", "user__full_name", "headline", "current_location", "skills_text")
	readonly_fields = ("created_at", "updated_at")


@admin.register(RecruiterProfile)
class RecruiterProfileAdmin(admin.ModelAdmin):
	list_display = ("user", "company_name", "job_title", "verified_company", "created_at")
	list_select_related = ("user",)
