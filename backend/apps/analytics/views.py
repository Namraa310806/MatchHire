...
class RecruiterDashboardView(APIView):
    ...
    def get(self, request):
        ...
        cache_key = CacheService.get_key(
            "dashboard_metrics", str(recruiter.id), "recruiter"
        )
        ...
        def compute_dashboard():
            ...
            application_metrics = Application.objects.filter(
                ...
                submitted_applications=Count(
                    "id", filter=Q(status=Application.ApplicationStatus.SUBMITTED)
                ),
                under_review_applications=Count(
                    "id", filter=Q(status=Application.ApplicationStatus.UNDER_REVIEW)
                ),
                shortlisted_applications=Count(
                    "id", filter=Q(status=Application.ApplicationStatus.SHORTLISTED)
                ),
                rejected_applications=Count(
                    "id", filter=Q(status=Application.ApplicationStatus.REJECTED)
                ),
                hired_applications=Count(
                    "id", filter=Q(status=Application.ApplicationStatus.HIRED)
                ),
            )
            ...
            interview_metrics = Interview.objects.filter(
                ...
                scheduled_interviews=Count(
                    "id", filter=Q(status=Interview.InterviewStatus.SCHEDULED)
                ),
                completed_interviews=Count(
                    "id", filter=Q(status=Interview.InterviewStatus.COMPLETED)
                ),
                cancelled_interviews=Count(
                    "id", filter=Q(status=Interview.InterviewStatus.CANCELLED)
                ),
            )
...
class CandidateDashboardView(APIView):
    ...
    def get(self, request):
        ...
        # PERFORMANCE OPTIMIZATION: Cache dashboard metrics
        cache_key = CacheService.get_key(
            "dashboard_metrics", str(candidate.id), "candidate"
        )
        ...
        def compute_dashboard():
            ...
            match_metrics = JobMatch.objects.filter(candidate=candidate).aggregate(
...
class JobAnalyticsView(APIView):
    ...
    def get(self, request, job_id):
        ...
        # Get application metrics in a single query
        application_metrics = Application.objects.filter(job=job).aggregate(
            total_applications=Count("id"),
            submitted=Count(
                "id", filter=Q(status=Application.ApplicationStatus.SUBMITTED)
            ),
...
class TopCandidatesView(APIView):
    ...
    def get(self, request, job_id):
        ...
        top_candidates = (
            ...
            .order_by("-match_score", "-calculated_at")[:TOP_CANDIDATES_LIMIT]
        )
