...
class JobDetailView(APIView):
    ...
    def patch(self, request, id):
        ...
        try:
            job = Job.objects.select_related("recruiter").get(
                id=id, recruiter=request.user
            )
        except Job.DoesNotExist:
            raise Http404("Job not found")
...
class JobCloseView(APIView):
    ...
    def get_object(self, request, id):
        ...
        validate_uuid(id, "id")
        try:
            return Job.objects.select_related("recruiter").get(
                id=id, recruiter=request.user
            )
        except Job.DoesNotExist:
            raise Http404("Job not found")
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            name="q",
            description="Search query (title, company, description)",
            required=False,
            type=str,
        ),
        OpenApiParameter(
        ...
        OpenApiParameter(
            ...
            description="Filter by experience level (entry, junior, mid, senior, lead)",
            required=False,
            type=str,
        ),
        OpenApiParameter(
            name="is_remote",
            description="Filter by remote status (true/false)",
            required=False,
            type=bool,
        ),
        OpenApiParameter(
        ...
        OpenApiParameter(
            name="ordering",
            description="Order by field (created_at, -created_at, salary_min, -salary_min, salary_max, -salary_max)",
            required=False,
            type=str,
        ),
        OpenApiParameter(
    ...
    def get(self, request):
        ...
        if employment_type:
            valid_employment_types = [
                choice[0] for choice in Job.EmploymentType.choices
            ]
        ...
        if experience_level:
            valid_experience_levels = [
                choice[0] for choice in Job.ExperienceLevel.choices
            ]
