...
class ResumeExtractionService:
    ...
    PORTFOLIO_PATTERN = (
        r"(?:https?://)?(?:www\.)?[a-zA-Z0-9-]+\.(?:com|io|net|org|dev|co|me)(?:/.*)?"
    )
    ...
    @classmethod
    def extract_summary(
        cls, raw_text: str, sections: Dict[str, Tuple[int, int]]
    ) -> str:
        """
    ...
    def extract_skills(
        ...
        if "skills" in sections:
            ...
            # Try bullet points
                ...
                skills = [
                    s.strip("•-").strip()
                    for s in section_text.split("\n")
                    if s.strip()
                    and (s.strip().startswith("•") or s.strip().startswith("-"))
                ]
            # Try newlines
            else:
                skills = [
            ...
            for entry in entries:
                ...
                for line in lines:
                    ...
                    if date_match:
                        ...
                        exp_data["start_date"] = cls._parse_date(
                            date_str.split("–")[0].split("-")[0].split("to")[0].strip()
                        )
                        if (
                            "–" in date_str
                            or "-" in date_str
                            or "to" in date_str.lower()
                        ):
                            end_part = (
                                date_str.split("–")[-1]
                                .split("-")[-1]
    ...
    @classmethod
    def _create_skills(
        cls, structured_resume: StructuredResume, skills: List[str]
    ) -> None:
    ...
    def _create_education(
        cls, structured_resume: StructuredResume, education_list: List[Dict]
    ) -> None:
    ...
    def _create_experience(
        cls, structured_resume: StructuredResume, experience_list: List[Dict]
    ) -> None:
    ...
    def _create_projects(
        cls, structured_resume: StructuredResume, project_list: List[Dict]
    ) -> None:
    ...
    def _create_certifications(
        cls, structured_resume: StructuredResume, certification_list: List[Dict]
    ) -> None:
