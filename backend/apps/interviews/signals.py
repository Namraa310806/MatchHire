...
def enqueue_interview_scheduled(sender, instance: Interview, created: bool, **kwargs) -> None:
...
def enqueue_interview_status_changed(sender, instance: InterviewStatusHistory, created: bool, **kwargs) -> None:
    if not created:
        return

    def enqueue() -> None:
        from apps.analytics.tasks import refresh_recruiter_analytics
        from apps.notifications.tasks import notify_interview_completed, notify_interview_cancelled
        ...
        refresh_recruiter_analytics.delay(str(instance.interview.application.job.recruiter_id))
