...
class SchemaGenerationTest(TestCase):
    ...
    def test_schema_has_tags(self):
        ...
        expected_tags = [
            'Authentication', 'Users', 'Profiles', 'Resumes',
            'Jobs', 'Applications', 'Matching', 'Interviews',
    ...
    def test_schema_has_enum_documentation(self):
        ...
        self.assertTrue(job_status_exists or len(schemas) > 0, "Schema should contain enum definitions")
    ...
    def test_schema_paths_are_valid(self):
        ...
        for path, methods in paths.items():
            ...
            for method, spec in methods.items():
                self.assertIn(method.lower(), ['get', 'post', 'put', 'patch', 'delete', 'options', 'head'])
                self.assertIn('responses', spec, f"{method.upper()} {path} should have responses")
